...
def train(config):
    ...
    # log all experimental config to wandb
    wandb.config.update(config)

    # reset seed for reproducability
    torch_utils.reset_seed(config.seed)

    # create random distributed train and dev dataset
    train_dataset = create_dataset(config.num_samples, config.device)
    dev_dataset = create_dataset(1000, config.device)
    # create dataloader for train and dev datasets
